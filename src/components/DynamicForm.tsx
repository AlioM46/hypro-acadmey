...
export default function DynamicForm({ activeCategory, setActiveCategory }: DynamicFormProps) {
  ...
  const syrianCitiesEn = ['Idlib', 'Aleppo', 'Hama', 'Latakia', 'Tartus', 'Damascus', 'Rif Dimashq', 'Homs', 'Daraa', 'As-Suwayda', 'Quneitra', 'Deir ez-Zor', 'Raqqa', 'Al-Hasakah'];
  const syrianCitiesAr = ['إدلب', 'حلب', 'حماة', 'اللاذقية', 'طرطوس', 'دمشق', 'ريف دمشق', 'حمص', 'درعا', 'السويداء', 'القنيطرة', 'دير الزور', 'الرقة', 'الحسكة'];
    ...
    switch (activeCategory) {
      case 'student':
        return [
          ...
          { id: 'city', labelEn: 'City / Governorate', labelAr: 'المحافظة / المدينة', type: 'select', optionsEn: syrianCitiesEn, optionsAr: syrianCitiesAr, required: true },
          { id: 'age', labelEn: 'Age', labelAr: 'العمر', type: 'number', placeholderEn: 'e.g. 24', placeholderAr: 'مثال: 24', required: false },
          { id: 'program', labelEn: 'Program of Interest', labelAr: 'البرنامج المطلوب', type: 'select', optionsEn: ['EV Technology Diploma', 'Hybrid Systems Diploma', 'Electronic Diagnostics Course', 'Other'], optionsAr: ['دبلوم تدريبي مهني في تكنولوجيا السيارات الكهربائية', 'دبلوم تدريبي مهني في الأنظمة الهجينة (الهايبرد)', 'دورة فحص إلكترونيات السيارات', 'أخرى'], required: true },
      ...
      case 'workshop':
        return [
          ...
          { id: 'city', labelEn: 'City / Location', labelAr: 'المحافظة / الموقع', type: 'select', optionsEn: syrianCitiesEn, optionsAr: syrianCitiesAr, required: true },
      ...
      case 'dealer':
        return [
          ...
          { id: 'city', labelEn: 'HQ Location', labelAr: 'موقع المقر الرئيسي', type: 'select', optionsEn: [...syrianCitiesEn, 'Jordan', 'Turkey', 'Other'], optionsAr: [...syrianCitiesAr, 'الأردن', 'تركيا', 'أخرى'], required: true },
      ...
      case 'ngo':
        return [
          ...
          { id: 'country', labelEn: 'Country of Operation', labelAr: 'بلد التشغيل', type: 'select', optionsEn: ['Syria', 'Jordan', 'Turkey', 'Lebanon', 'Iraq', 'Multi-country', 'Other'], optionsAr: ['سوريا', 'الأردن', 'تركيا', 'لبنان', 'العراق', 'متعدد الدول', 'أخرى'], required: true },
          { id: 'missionGoal', labelEn: 'Partnership Objective', labelAr: 'هدف الشراكة', type: 'select', optionsEn: ['Fund vocational scholarships', 'Train institutional technicians', 'Establish community labs', 'Research & impact assessment', 'Other'], optionsAr: ['تمويل منح تدريب مهني', 'تدريب فنيي المؤسسات', 'إنشاء مختبرات مجتمعية', 'بحث وتقييم أثر', 'أخرى'], required: true },
      ...
      case 'trainer':
        return [
          ...
          { id: 'city', labelEn: 'City / Location', labelAr: 'المحافظة / المدينة', type: 'select', optionsEn: syrianCitiesEn, optionsAr: syrianCitiesAr, required: true },
          { id: 'specialty', labelEn: 'Specialty / Domain', labelAr: 'التخصص التدريبي', type: 'select', optionsEn: ['Electric & Hybrid Vehicles', 'Automotive Electronics', 'High-Voltage Battery Diagnostics', 'Alternative Energy Systems', 'Diagnostics & Scan Tools', 'Other'], optionsAr: ['السيارات الكهربائية والهجينة', 'إلكترونيات وكهرباء السيارات', 'تشخيص وصيانة بطاريات الجهد العالي', 'أنظمة الطاقة البديلة والمتجددة', 'أجهزة الفحص وتشخيص الأعطال', 'أخرى'], required: true },
  ...
  const handleSubmission = async (e: React.FormEvent) => {
    ...
    if (formData.age) {
      ...
      if (isNaN(ageVal) || ageVal < 15 || ageVal > 90) {
        const ageError = lang === 'en'
          ? 'Please enter a valid age between 15 and 90.'
          : 'يرجى إدخال عمر صحيح بين 15 و 90 عاماً.';
        alert(ageError);
        return;
      }
    }
