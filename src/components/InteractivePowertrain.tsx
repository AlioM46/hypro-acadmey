  ...
  return (
    <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-200 shadow-sm transition-all animate-fade-in" id="powertrain-simulator">
        ...
        <div className="flex flex-wrap lg:flex-nowrap gap-1 bg-slate-100 p-1 rounded-xl w-full lg:w-auto">
          {(['fuel', 'hybrid', 'ev'] as DriveMode[]).map((mode) => (
            <button
              ...
              id={`btn-mode-${mode}`}
              className={`flex-1 lg:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer border-none ${
                activeMode === mode
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-7 bg-slate-50 border border-slate-200 rounded-xl p-4 md:p-6 overflow-hidden relative flex flex-col justify-between aspect-video min-h-[290px]">
          ...
          >
            ...
            <g opacity={activeMode === 'ev' ? 0.2 : 1.0} className="transition-opacity duration-300">
              ...
              <foreignObject x="34" y="44" width="102" height="62">
                <div className="w-full h-full flex flex-col items-center justify-center text-center p-1 font-sans">
                  <span className={`text-[10px] sm:text-[11px] font-bold ${activeMode !== 'ev' ? 'text-slate-800' : 'text-slate-400'}`}>
                    {components.fuelTank}
                  </span>
                </div>
              </foreignObject>
            ...
            <g opacity={activeMode === 'ev' ? 0.2 : 1.0} className="transition-opacity duration-300">
              ...
              <foreignObject x="214" y="44" width="112" height="62">
                <div className="w-full h-full flex flex-col items-center justify-center text-center p-1 font-sans">
                  <span className={`text-[10px] sm:text-[11px] font-bold ${activeMode !== 'ev' ? 'text-slate-900' : 'text-slate-400'}`}>
                    {components.engine}
                  </span>
                </div>
              </foreignObject>
            ...
            <g opacity={activeMode === 'fuel' ? 0.2 : 1.0} className="transition-opacity duration-300">
              ...
              <foreignObject x="34" y="212" width="122" height="34">
                <div className="w-full h-full flex flex-col items-center justify-center text-center px-1 font-sans">
                  <span className={`text-[10px] sm:text-[11px] font-bold ${activeMode !== 'fuel' ? 'text-slate-800' : 'text-slate-400'}`}>
                    {components.battery}
                  </span>
                </div>
              </foreignObject>
            ...
            <g opacity={activeMode === 'fuel' ? 0.2 : 1.0} className="transition-opacity duration-300">
              ...
              <foreignObject x="414" y="179" width="112" height="62">
                <div className="w-full h-full flex flex-col items-center justify-center text-center p-1 font-sans">
                  <span className={`text-[10px] sm:text-[11px] font-bold ${activeMode !== 'fuel' ? 'text-slate-900' : 'text-slate-400'}`}>
                    {components.motor}
                  </span>
                </div>
              </foreignObject>
          ...
          <div className="z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between border-t border-slate-200 pt-3 text-[10px] font-mono text-slate-500 gap-1.5 w-full text-start">
